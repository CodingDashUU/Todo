...
where TDbContext : ApplicationDbContext
{
    ...
    private ChallengeHttpResult ChallengeExternal([FromQuery] string returnUrl)
    {
        var properties =
            signInManager.ConfigureExternalAuthenticationProperties(
                GoogleDefaults.AuthenticationScheme,
                $"{CoreEndpointRoutes.External.ExternalCallback}?ReturnUrl={returnUrl}");
    ...
    private async Task<IResult> ExternalCallback([FromQuery] string returnUrl)
    {
        var info = await signInManager.GetExternalLoginInfoAsync();
        if (info is null) return Results.Redirect($"{ApplicationRoutes.SignIn}?ReturnUrl={returnUrl}");
